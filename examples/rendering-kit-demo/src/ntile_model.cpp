...
namespace ntile_model
{
    ...
    CharacterModel::CharacterModel(ErrorBuffer_t* eb, IResourceManager* res)
    {
        this->eb = eb;
        this->res = res;
    ...
    bool CharacterModel::AddCuboid(MeshBuildContext_t* ctx, cfx2::Node cuboidNode)
    {
        ...
        if (bone != nullptr)
        {
            ...
            if (joint == nullptr)
                client::g_sys->Log(kLogWarning, "CharacterModel: Ignoring invalid bone '%s' for primitive '%s'\n", bone, cuboidNode.getText());
            else
            {
    ...
    bool CharacterModel::Load(const char* path)
    {
        ...
        program = client::irm->GetSharedResourceManager()->GetResourceByPath<IShader>("RenderingKit/basicTextured", RESOURCE_REQUIRED, 0);
        zombie_assert(program);

        vfRef = client::irm->CompileVertexFormat(program.get(), sizeof(ModelVertex), modelVertexAttribs, false);
        vbRef = client::irm->CreateGeomBuffer(path);

        auto texture = res->GetResource<RenderingKit::ITexture>("path=ntile/worldtex_0.png", 0, RESOURCE_REQUIRED);
        ...
        this->materialRef = client::irm->CreateMaterial("basicTextured", program);
        ...
        /*Reference<SeekableInputStream> is = Sys::OpenInput(filename);
        ...
        cfx2_Node* docNode = cfx2_load_document(path);
        //ZFW_ASSERT(cfx2_read_from_string(contents, &docNode) == cfx2_ok)
        ZFW_ASSERT(docNode);
        ...
        {
            client::g_sys->Log(kLogError, "CharacterModel loading - model '%s'", path);
            return false;
        }
    ...
    {
        client::irm->DrawPrimitives(materialRef.get(), RK_LINES, skeletonMesh);
    }
