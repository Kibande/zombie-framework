...
namespace ntile
{
    ...
    void GameScreen::SetupWorldLighting(const glm::mat4x4& modelView, Float3& backgroundColour)
    {
        ...
        backgroundColour = sunColour * sunLight.ambient;

        if (sun_amb >= 0)
            worldShader->SetUniformVec3(sun_amb, sunColour * sunLight.ambient);
        ...
        if (sunAngle > 0.0f && sunAngle < f_pi)
        {
            ...
            worldShader->SetUniformVec3(sun_dir, -sunDir);
            worldShader->SetUniformVec3(sun_diff, sunColour * (1.0f - sunLight.ambient));

            backgroundColour += sunColour * (1.0f - sunLight.ambient);
        }
        else
            // deep night, no sun diffuse needed
            worldShader->SetUniformVec3(sun_diff, Float3(0.0f, 0.0f, 0.0f));
    }
