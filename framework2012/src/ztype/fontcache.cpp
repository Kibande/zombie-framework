...
namespace ztype
{
    ...
    {
		return lodepng::encode(filename, (const uint8_t*) pixels, width, height, LCT_RGBA) == 0;
    }
    ...
    bool FontCacheImpl::IsUpToDate(const String& cache_filename, const FaceDesc *entry)
    {
        ...
        Object<FaceFile> faceFile = FaceFile::Open(cache_filename);
    ...
    int FontCacheImpl::UpdateCache(bool verbose)
    {
        ...
        int num_faces = updateList.getLength();

        iterate2 (iter, updateList)
        {
            UpdateEntry& entry = iter;

            int curr_face = iter.getIndex();

            if (progressListener != nullptr)
                progressListener->OnFontCacheProgress(curr_face, num_faces, 0, entry.desc.range_max - entry.desc.range_min);
            ...
            Reference<File> output = File::open(entry.cache_filename, true);
            ...
            output->write<uint32_t>(0xCECA87F0);

            // 3 sections at fixed positions (Hash: 8 + 24, Metr: 8 + 24 + 16, FTex: 8 + 24 + 16 + 8)
            output->write<uint32_t>(3);

            output->write("Hash", 4);
            output->write<uint32_t>(32);

            output->write("Metr", 4);
            output->write<uint32_t>(48);

            output->write("FTex", 4);
            output->write<uint32_t>(56);

            // Hash section
            output->write(md5, 16);

            // Metr section
            output->writeItems(rasterizer->GetMetrics(), 1);

            // FTex section
            // only 1 range is currently supported
            // this will need to be changed eventually
            output->write<uint32_t>(1);
            output->write<uint32_t>(entry.desc.range_min);
            output->write<uint32_t>(numGlyphs);
            ...
            for (uint32_t i = 0; i < numGlyphs; i++)
            {
                ...
                if (progressListener != nullptr && !(i & 0x1F))
                    progressListener->OnFontCacheProgress(curr_face, num_faces, i, numGlyphs);
